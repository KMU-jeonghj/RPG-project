...
public class Event {
	...
	void switchEvent(int year) //Game 클래스의 year
	{		
		switch(year)
		{
		case 1985:
		...
		case 2002:
			eventWordCup(); break;
		case 2008:
			eventChoiceLife(); break;		
		}
	}
	
	void eventSong(){
		WedRose wedRose;
		wedRose.song(); // 만들었는데 전체 코드 고려 안 하고 이름 붙여서 다시 수정해야함
	}
	...
	void eventWar() {
		...
		int event90 = input.nextInt(); //Hero객체에서 public으로 선언한 스캐너 
		int minusMoney;
		
		if (event90 == 1){
			gangster.gangCnt -= (int)(gangster.gangCnt * 0.3);
			System.out.println("무리한 활동으로 인해 조직원의 30%가 수감되었습니다.");
			Thread.sleep(100);
			System.out.println("현재 나의 조직원 수 :" + gangster.gangCnt + "명");
			Thread.sleep(100);
			
			minusMoney = (int)(hero.money * 0.15);
			hero.money -= minusMoeny;
			System.out.println("무리한 활동으로 인해 벌금 " + minusMoney + "이 부과되었습니다.");
			Thread.sleep(100);
			System.out.println("현재 나의 재산 :" + hero.Money + "원");
		}
		
		else if(event90 == 2) {
			gangster.gangCnt -= (int)(gangster.gangCnt * 0.1);
			System.out.println("당신의 유연한 대처로 조직원의 10%만이 수감되었습니다.");
			Thread.sleep(100);
			System.out.println("현재 나의 조직원 수 :" + gangster.gangCnt + "명");
			Thread.sleep(100);
			
			minusMoney = (int)(hero.money * 0.07);
			hero.money -= minusMoeny;
			System.out.println("무리한 활동으로 인해 벌금 " + minusMoney + "이 부과되었습니다.");
			Thread.sleep(100);
			System.out.println("현재 나의 재산 :" + hero.Money + "원");
	...
	void eventIMF(){
		...
		Thread.sleep(100);
		System.out.println("모두가 어려운 시기, 아직은 살만한 " + hero.name + "은 이자를 걷을지 말지 고민중이다.");
		...
		if(choice97 == 1){
			System.out.println("어려운 시기였지만 "+ hero.name +"의 재산은 상승세였다.\n"
					+ "그런 " +hero.name+ "를 손가락질하는 이들 또한 생겨났다.");

		}
		else if (choice97 == 2)
		{
			hero.money -= (int)(hero.money * 0.1);
			System.out.println(hero.name +"의 재산이 10% 감소했다.\n"
				+ "그런 " +hero.name+ "를 우러러보는 이들이 생겨났다.");
			System.out.println("현재 나의 재산 :" + hero.Money + "원");
		}
	...
	void eventWordCup() {
		...
		if(choice02 == 1){
			...
			System.out.prinltn(hero.name +"의 재산이 30% 증가했다.");
		...
		{
			System.out.println("2002년 6월 22일 한국은 스페인을 꺾고 4강에 진출했다. ");
			System.out.prinltn(hero.name +"의 재산이 5% 감소했다.");
			
			hero.money -= (int)(hero.money * 0.05);
		}
		else if (choice03 == 3)
		{
			System.out.println("2002년 6월 22일 한국은 스페인을 꺾고 4강에 진출했다. ");
	...
	void eventchoiceLife() {
		System.out.println("2008년, " + hero.name + "에게 뜻밖의 제안이 들어온다\n"
